...
function FeatureMockup({ tag }: { tag: string }) {
  return (
    <div className="w-full max-w-xs bg-white border border-warm-gray/30 rounded-3xl p-5 shadow-sm flex flex-col gap-3">
      <div className="text-xs font-semibold text-warm-gray tracking-widest uppercase">{tag}</div>
      <div className="flex flex-col gap-2">
        <div className="self-end bg-denim rounded-2xl px-3 py-2 max-w-[75%]">
          <div className="h-2.5 w-24 bg-white/60 rounded-full" />
        </div>
        <div className="self-start bg-cream border border-warm-gray/30 rounded-2xl px-3 py-2 max-w-[85%]">
          <div className="h-2.5 w-32 bg-black/30 rounded-full mb-1.5" />
...
function FeatureRow({ s, i }: { s: FeatureSection; i: number }) {
...
export function Features() {
  return (
    <section id="features" className="bg-cream">
      {sections.map((s, i) => (
        <FeatureRow key={s.tag} s={s} i={i} />
      ))}

      <footer className="bg-black text-cream py-12 px-6 md:px-12">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-denim rounded-xl flex items-center justify-center">
