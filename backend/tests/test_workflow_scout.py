...
def _make_request(scout: str = "force") -> dict:
    return {
        "mode": "workflow",
        "query": "test query",
        "workflow_nodes": [{"id": "n0", "type": "model", "model": "Swift", "inputs": []}],
        "scout": scout,
    }


def test_workflow_scout_force_emits_tool_call_and_result_before_hex_start(
    monkeypatch: pytest.MonkeyPatch,
    _base_overrides: tuple,
) -> None:
    """scout=force: tool_call + tool_result before first hex_start."""
    ...
    with patch("app.workflow.executor.scout_force", new=AsyncMock(return_value=("ctx", fake_result))):
        with patch("app.workflow.executor.get_client", return_value=FakeClient(["answer"])):
...
def test_workflow_scout_auto_also_does_pre_injection(
    monkeypatch: pytest.MonkeyPatch,
    _base_overrides: tuple,
) -> None:
    """scout=auto: same pre-injection as force in workflow."""
    import app.api.query as query_module
...
def test_workflow_scout_off_no_tool_events(
    monkeypatch: pytest.MonkeyPatch,
    _base_overrides: tuple,
) -> None:
    """scout=off: no tool events."""
    import app.api.query as query_module
...
def test_workflow_scout_missing_key_returns_error_and_done(
    monkeypatch: pytest.MonkeyPatch,
    _base_overrides: tuple,
) -> None:
    ...
    with patch("app.workflow.executor.scout_force", new=AsyncMock(side_effect=ValueError("TAVILY_API_KEY not set"))):
        with TestClient(app) as tc:
            r = tc.post("/api/query", json=_make_request("force"))
