...
async def _execute_model_node(
    ...
    """Stream a model node: hex_start → tokens → hex_done. Yields SSE bytes.

    On unknown model or streaming error, yields error event and returns.
    Output text is collected and returned via generator send protocol — we
    instead track it through a mutable list captured in the closure.
    """
    ...
    messages: list[LLMMessage] = [
        LLMMessage(role="user", content=node_input),
    ]
    ...
    try:
        ...
        logger.exception("workflow: model node %r (%s) stream failed", node_id, whitelabel)
        yield format_event(SseEvent(
            "error",
    ...
    # Signal the output text back via a special sentinel appended to collected
    # We use a side-channel dict injected by the caller instead.
    # (See _workflow_stream for how output is captured.)
